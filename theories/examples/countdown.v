...
Section handlee_verification.
  ...
  Lemma get_refines :
    ⊢ REL get timer1 #() ≤
          ...
          {{v1; v2, ∃ (x : Z), ⌜ v1 = #x ⌝ ∗ ⌜ v2 = #x ⌝}}.
  Proof.
    rewrite /get. rel_pures_l. rel_pures_r.
    ...
    iIntros "!> %x". iApply rel_value. by iExists x.
  Qed.
  ...
  Lemma set_refines (x : Z) :
    ⊢ REL set timer1 #x ≤
          ...
          {{v1; v2, ⌜ v1 = #() ⌝ ∗ ⌜ v2 = #() ⌝}}.
  Proof.
    rewrite /set. rel_pures_l. rel_pures_r.
    iApply rel_introduction'.
    iExists _, _, [], [], _.
    do 2 (iSplit; [done|]; iSplit; [iPureIntro; apply _|]); simpl.
    iSplitL; [|by iIntros "!>" (??) "H"; iApply "H"].
    iRight. iExists x. do 2 (iSplit; [done|]).
    iModIntro. by iApply rel_value.
  Qed.

  Lemma while_refines :
    ⊢ REL while (get timer1) (set timer1) ≤
  ...
  Proof.
    ...
    iApply rel_wand. { by iApply get_refines. }
    iIntros "!> %% [%x [-> ->]]".
    rel_pures_l. rel_pures_r.
    ...
    - intros Hnneg Hdec. rewrite bool_decide_eq_false_2; [|done].
      ...
      iApply (rel_bind' [_] [_]).
      { by iApply traversable_iThyTraverse; apply _. }
      iApply rel_wand. { by iApply set_refines. }
      iIntros "!> %% [-> ->]".
  ...
  Lemma countdown_refines :
    ⊢ REL countdown (get timer1) (set timer1) ≤
          countdown (get timer2) (set timer2)
          <|iThyTraverse [timer1] [timer2] TimerRefl|>
          {{v1; v2, ⌜ v1 = v2 ⌝}}.
  Proof.
    rewrite /countdown. rel_pures_l. rel_pures_r.
    iApply (rel_bind' [AppRCtx _] [AppRCtx _]).
    { by iApply traversable_iThyTraverse; apply _. }
    iApply rel_wand. { by iApply set_refines. }
    iIntros "!> %% [-> ->]". simpl. do 2 rel_pure_l. do 2 rel_pure_r.
...
Section handler_verification.
  ...
  Lemma run_st_passing_handler_refines_run_heap_handler
    ...
    REL e1 ≤ e2 <|Timer|> {{v1; v2, ⌜ v1 = v2 ⌝}} -∗
    l2 ↦ₛ  #init -∗
    REL run_st_passing_handler timer1 e1 #init ≤
  ...
  Proof.
    ...
    iApply (rel_exhaustion [_; _] [_] with "He").
    iSplit; [by iIntros (??) "->"; rel_pures_l; rel_pures_r|].
    clear e1 e2.
    iIntros (e1 e2 ?)
      "[%e1' [%e2' [%k1 [%k2 [%S
  ...
  Lemma run_st_passing_refines_run_heap
    ...
    let Timer := iThyTraverse [timer1] [timer2] (TimerRefl timer1 timer2) in
    REL main1 #() ≤ main2 #() <|Timer|> {{v1; v2, ⌜ v1 = v2 ⌝}} -∗
    REL run_st_passing timer1 #init main1 ≤
  ...
  Proof.
    ...
    rel_pures_l. rel_pures_r.
    iApply rel_alloc_r. iIntros (l2) "Hl2". rel_pures_r.
...
Section handler_verification.
  ...
  Lemma run_st_passing_handler_refines (l : loc) (e1 e2 : expr) (init : Z) :
    l ↦ₛ {# 1/2} #init -∗
    REL e1 ≤ e2 <|iThyTraverse [timer] [] (Timer timer l)|> {{ v1 ; v2, ⌜ v1 = v2 ⌝ }} -∗
  ...
  Lemma run_st_passing_refines l (main1 : val) (main2 : expr) (init : Z) :
    l ↦ₛ {# 1/2} #init -∗
    REL main1 #() ≤ main2 <|iThyTraverse [timer] [] (Timer timer l)|> {{v1; v2, ⌜ v1 = v2 ⌝}} -∗
    REL run_st_passing timer #init main1 ≤ main2 <|iThyBot|> {{v1; v2, ⌜ v1 = v2 ⌝}}.
  Proof.
